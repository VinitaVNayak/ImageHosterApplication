...
public class ImageRepository {
    ...
    // the image is displayed with title corresponding to particular imageId as well as title and not just the title
    public Image getImageByTitle(String title, Integer imageId) {
        EntityManager em = emf.createEntityManager();
        try {
            TypedQuery<Image> typedQuery = em.createQuery("SELECT i from Image i where i.title =:title AND i.id=:imageId", Image.class).setParameter("imageId", imageId).setParameter("title", title);
